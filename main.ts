(() => {
  const editJson = document.getElementById("editJson") as HTMLButtonElement;

  editJson.addEventListener("click", () => {
    window.location.href = "edit.html";
  });

  let json: any[] = JSON.parse(localStorage.getItem("json") || "[]");
  ...
  (async () => {
    ...
    const clearCustomList = document.getElementById("clearCustomList") as HTMLButtonElement;
    ...
    async function displayItems() {
      ...
      for (const category of Object.keys(json)) {
        ...
        for (const subcategory of Object.keys(json[category])) {
          const subcategoryItem = document.createElement("li");
          subcategoryItem.innerText = subcategory;
    
          const itemsList = document.createElement("ul");
    
          for (const item of json[category][subcategory]) {
        ...
        inventoryList.appendChild(categoryItem);
      }
    }
  
    clearCustomList.addEventListener("click", () => {
  ...
  function populateMainCategories(menuData: CountData): void {
    ...
    // Call populateSubCategories with appropriate SubCategoryData
    populateSubCategories(menuData[mainCategoryElement.value]);
  }

  function populateSubCategories(mainCategoryData: MainCategoryData): void {
    subCategoryElement.innerHTML = '';
    Object.keys(mainCategoryData).forEach(key => {
      const optionItem = document.createElement('option');
      optionItem.textContent = key;
    ...
    subCategoryElement.addEventListener('change', () => {
      populateItems(mainCategoryData[subCategoryElement.value]);
    });
